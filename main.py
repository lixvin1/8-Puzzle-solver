...
class DFSSearch:
    def solve(self, start_state):
        ...
        frontier = [start_state]#stack initialize nad the start state appended
        frontier_boards = {start_state.board}
        explored = set() #set iniatliezed for visited states

        nodes_expanded = 0
        max_fringe_size = 1#for forst state
        max_search_depth = 0
        ...
        while frontier:
            ...
            if current_state.is_goal():#goal was reached
                running_time = time.time() - start_time
                current_mem, peak_mem = tracemalloc.get_traced_memory()
                tracemalloc.stop()
            ...
            for neighbor in reversed(neighbors): #to have same order of actions with popping from the end
                if neighbor.board not in frontier_boards and neighbor.board not in explored:
                    frontier.append(neighbor)
                    frontier_boards.add(neighbor.board)
...
class DFSDepthLimitedSearch:
    ...
    def solve(self, start_state):
        ...
        while frontier:
            ...
            if current_state.depth < self.limit:#check limit is not exceeded
                nodes_expanded += 1
...
class IDSearch:
    def solve(self, start_state):
        ...
        while True:
            dls = DFSDepthLimitedSearch(limit)#calling dls with increasing limit until solution is found
            result = dls.solve(start_state)
            ...
            if result is not None:
                ...
                result["running_time"] = total_time
                result["max_ram_usage"] = peak_mem / (1024 * 1024)
                return result
...
def main():
    ...
    # limit = 2
    # dls_strategy = DFSDepthLimitedSearch(limit)
    # solver = PuzzleSolver(dls_strategy)

    ids_strategy = IDSearch()
    solver = PuzzleSolver(ids_strategy)
